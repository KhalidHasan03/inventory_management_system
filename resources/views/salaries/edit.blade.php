...
<div class="bg-dark-800 rounded-2xl border border-dark-700 p-6">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-xl font-bold text-white">Edit Salary</h2>
                <p class="text-dark-400 text-sm">Update salary information</p>
            </div>
        </div>
    ...
    <form action="{{ route('salaries.update', $salary->id) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
                ...
                <select name="employee_id" required class="classic-select">
                    ...
                    <option value="{{ $employee->id }}" {{ $salary->employee_id == $employee->id ? 'selected' : '' }}>
                        {{ $employee->user->name }} - ${{ number_format($employee->salary, 2) }}/month
                    </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="number" name="amount" step="0.01" value="{{ $salary->amount }}" required class="classic-input">
            </div>
