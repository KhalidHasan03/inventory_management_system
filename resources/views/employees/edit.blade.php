    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-dark-400 text-sm">Update employee information</p>
            </div>
        </div>
    ...
    <form action="{{ route('employees.update', $employee->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
                ...
                <input type="text" name="name" value="{{ $employee->user->name }}" required class="classic-input">
            </div>

            <div class="space-y-2">
                <label class="text-sm font-medium text-dark-300">Email Address</label>
                <input type="email" name="email" value="{{ $employee->user->email }}" required class="classic-input">
            </div>

            <div class="space-y-2">
                <label class="text-sm font-medium text-dark-300">Phone Number</label>
                <input type="text" name="phone" value="{{ $employee->user->phone }}" required class="classic-input">
            </div>

            <div class="space-y-2">
                <label class="text-sm font-medium text-dark-300">Monthly Salary ($)</label>
                <input type="number" name="salary" step="0.01" value="{{ $employee->salary }}" required class="classic-input">
            </div>

            <div class="space-y-2">
                <label class="text-sm font-medium text-dark-300">Join Date</label>
                <input type="date" name="join_date" value="{{ $employee->join_date }}" required class="classic-input">
            </div>

            <div class="space-y-2">
                <label class="text-sm font-medium text-dark-300">New Password (leave blank)</label>
                <input type="password" name="password" class="classic-input" placeholder="Enter new password">
            </div>

            <div class="space-y-2 md:col-span-2">
                <label class="text-sm font-medium text-dark-300">Address</label>
                <textarea name="address" rows="2" class="classic-input">{{ $employee->user->address }}</textarea>
            </div>
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <img src="{{ asset('uploads/employees/' . $employee->profile_pic) }}" class="w-16 h-16 rounded-lg object-cover border-2 border-green-500">
                    @endif
