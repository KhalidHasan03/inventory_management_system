...
<div class="bg-dark-800 rounded-2xl border border-dark-700 p-6">
    ...
    <form method="GET" class="flex flex-wrap gap-4 mb-6 p-4 bg-dark-900 rounded-xl">
        ...
        <input type="date" name="start_date" value="<?php echo e(request('start_date')); ?>" class="classic-input" style="width: auto; min-width: 150px;" placeholder="Start Date">
        <input type="date" name="end_date" value="<?php echo e(request('end_date')); ?>" class="classic-input" style="width: auto; min-width: 150px;" placeholder="End Date">
        <button type="submit" class="classic-btn" style="padding: 10px 20px;">
            <i class="fas fa-filter mr-2"></i>Filter
        </button>
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(request('status') || request('start_date') || request('end_date')): ?>
        <a href="<?php echo e(route('orders.index')); ?>" class="classic-btn-secondary" style="padding: 10px 16px;">
                ...
                <tr class="hover:bg-dark-700/30 transition-smooth">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
                                <span class="text-white text-xs font-bold"><?php echo e(substr($order->customer->name, 0, 1)); ?></span>
                            </div>
                            <span class="text-dark-300"><?php echo e($order->customer->name); ?></span>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <span class="text-white font-bold">$<?php echo e(number_format($order->grand_total, 2)); ?></span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <span class="text-emerald-400 font-medium">$<?php echo e(number_format($order->paid_amount, 2)); ?></span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <span class="text-<?php echo e($order->due_amount > 0 ? 'red' : 'dark-400'); ?>-400 font-medium">$<?php echo e(number_format($order->due_amount, 2)); ?></span>
                    </td>
                    <td class="px-6 py-4">
                        <span class="px-2.5 py-1 rounded-lg text-xs font-medium bg-dark-700 text-dark-300 capitalize"><?php echo e($order->payment_method); ?></span>
                    </td>
                    <td class="px-6 py-4">
                        <span class="px-3 py-1.5 rounded-lg text-xs font-semibold
                            ...
                            <i class="fas <?php echo e($order->status === 'completed' ? 'fa-check' : ($order->status === 'pending' ? 'fa-clock' : 'fa-times')); ?> mr-1"></i>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-2">
                            <a href="<?php echo e(route('orders.show', $order->id)); ?>" class="p-2.5 rounded-lg hover:bg-blue-500/20 text-blue-400 transition-smooth" title="View Details">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a href="<?php echo e(route('orders.invoice', $order->id)); ?>" class="p-2.5 rounded-lg hover:bg-green-500/20 text-green-400 transition-smooth" title="Print Invoice">
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\inventory_manage\inventory_management_system\resources\views/orders/index.blade.php ENDPATH**/ ?>